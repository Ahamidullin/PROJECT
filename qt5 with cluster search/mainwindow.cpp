...
{
    ui->setupUi(this);
    connect(ui->searchClusterButton, &QPushButton::clicked, this, &MainWindow::on_searchClusterButton_clicked);  // Connect the slot
}
...
void MainWindow::on_clusterButton_clicked() {
    ...
    if (ok) {
        ...
        ui->tableWidget->setColumnCount(9);  // Increased column count by 1
        ...
        for (const auto &neo : neos) {
            ui->tableWidget->setItem(row, 0, new QTableWidgetItem(QString::number(neo.cluster_id)));  // Cluster ID
...
void MainWindow::on_searchClusterButton_clicked() {
    ...
    if (ok) {
        ...
        ui->tableWidget->clearContents();  // Clear existing contents
